...
TRUNCATE TABLE participa_em CASCADE;
DROP TABLE participa_em CASCADE;
-- tabela participa_em
...
-- 1. Alunos e disciplinas obrigatórias do semestre 2
WITH alunos_sem2 AS (
    SELECT
        A.nusp_usuario AS nusp_aluno,
        G.sigla_disciplina,
...
-- 2. Contar alunos por turma para lotação
...
-- 3. Distribuir alunos por turma menos cheia
turmas_disponiveis AS (
    SELECT
        ad.nusp_aluno,
        dt.sigla_disciplina,
...
-- 4. Inserir no participa_em apenas matrícula (sem notas)
...
-- Insere um monitor aleatório (que NÃO ESTÁ matriculado nela) para cada turma do semestre 2025-2
...
WITH turmas_semestre AS (
    -- 1. Seleciona todas as turmas alvo que precisam de monitor
    SELECT 
        sigla_disciplina,
        codigo_turma,
        vigencia
    FROM 
        turma
    WHERE 
        vigencia = '2025-2' -- Filtre pelo semestre desejado
),
monitores_por_turma AS (
    -- 2. Combina cada turma com TODOS os monitores elegíveis
    SELECT
        t.sigla_disciplina,
        t.codigo_turma,
        t.vigencia,
        m.nusp_aluno AS nusp_monitor,
        
        -- Garante que o monitor NÃO esteja matriculado na turma (o LEFT JOIN falha = NULL)
        mcr.nusp_aluno AS nusp_matriculado_na_turma,
        
        -- 3. Atribui um rank aleatório para selecionar apenas um monitor por turma
        ROW_NUMBER() OVER (
    ...
    FROM
        turmas_semestre t
    CROSS JOIN
        monitor m -- Combina todas as turmas com todos os monitores
    LEFT JOIN
        matricula mcr ON mcr.nusp_aluno = m.nusp_aluno -- Tenta achar a matricula do monitor na turma
                      AND mcr.sigla_disciplina = t.sigla_disciplina
                      AND mcr.codigo_turma = t.codigo_turma
                      AND mcr.vigencia = t.vigencia
    WHERE
        mcr.nusp_aluno IS NULL -- <--- FILTRO CHAVE: EXCLUI quem está matriculado na turma
)
-- 4. Seleciona o monitor de rank 1 (o aleatório)
SELECT
    80 AS carga_horaria, -- Define uma carga horária padrão (80h)
    nusp_monitor,
    sigla_disciplina,
    codigo_turma,
    vigencia
...
DROP TABLE avaliacao_sala CASCADE;
...
TRUNCATE TABLE leciona CASCADE;
...
SELECT
    -- Seleciona o NUSP do professor com o menor valor
    MIN(v.nusp_associado) AS nusp_professor,
    
    -- ID do Departamento (agrupador)
    d.id_departamento,
...
FROM
    vinculo_trabalho v
JOIN
    departamento d ON v.id_instituto = d.id_instituto
JOIN
    professor p ON v.nusp_associado = p.nusp_associado
WHERE
    v.cargo LIKE 'Professor Nível 1%'  -- <--- CORREÇÃO: Removi o espaço final e usei %
GROUP BY
    d.id_departamento;

DROP TABLE oferta_aula CASCADE;
-- tabela oferta_aula
...
FROM
    ofertas_numeradas;
TRUNCATE TABLE pedido_reserva CASCADE;
